...
```bash
make install     # uv sync --extra dev + pre-commit install (ajouter --extra rag pour les SDK anthropic/voyage/cohere)
...
middleware.py  rate limiting sliding-window par IP sur les routes RAG (enregistré dans create_app)
dependencies.py  dépendances FastAPI injectables (get_principal : auth par X-API-Key) — surchargeables en test
governance.py  RBAC / mask_pii / IdempotencyGuard / AuditLog (couche transverse)
...
- **Config fail-fast** : `Settings._validate_production_safety` (un `@model_validator` pydantic) refuse de démarrer sur des combinaisons non sûres (ex. `provider=http` sans base URL). Ajouter les nouveaux invariants ici, pas dans les handlers de requêtes. `get_settings()` (`@lru_cache(maxsize=1)`) est l'accès caché utilisé par le composition root ; en test, surcharger via `get_settings.cache_clear()` ou en passant un `Settings` explicite à `build_container`.
...
- **Probes** : `/healthz` est un liveness O(1) (aucune dépendance). `/readyz` est un readiness qui sonde la dépendance avec état du service via `RagService.ready()` → `VectorStorePort.ready()` (un ping léger, ex. `SELECT 1` pour un futur adapter pgvector ; le fake en mémoire renvoie `True`). Contrat : `ready()` **ne lève jamais** et renvoie `False` si le backend est injoignable. Les APIs LLM (Voyage/Claude/Cohere) ne sont **pas** sondées ici : leurs pannes transitoires sont gérées par timeout/retry par requête, et les poller à chaque sonde serait coûteux/nuisible.
- **Authentification** : les routes `/rag/*` exigent un en-tête `X-API-Key`. `dependencies.get_principal` le résout en `Principal` via `Settings.api_key_mapping` (paires `clé:rôle` issues de `APP_API_KEYS`, défaut `dev-key-viewer:viewer` / `dev-key-operator:operator`). Header manquant → 422 (header requis FastAPI) ; clé inconnue → 401. `get_principal` est une dépendance injectable, surchargeable en test via `app.dependency_overrides`.
- **Rate limiting** : `middleware.setup_rate_limiting` pose un limiteur sliding-window **par IP** sur `/rag/index` et `/rag/query` (`APP_RATE_LIMIT_MAX_REQUESTS` / `APP_RATE_LIMIT_WINDOW_SEC`, défaut 60/60s) ; dépassement → 429. État en mémoire ; les tests l'isolent via `middleware.reset_rate_limiter()`.
- **Gouvernance** : `app.governance` fournit `RBACPolicy`/`Permission`/`Principal` (RBAC), `mask_pii`, `IdempotencyGuard` et `AuditLog`, tous vivant dans le `Container`. **RBAC, audit et idempotence sont câblés** : `/rag/index` exige `Permission.WRITE` (rôle `operator`), `/rag/query` exige `Permission.READ` (rôle `viewer` ou `operator`) ; un refus lève `AccessDeniedError` → 403 (handler dans `api/app.py`). `/rag/index` enregistre une entrée d'audit et accepte un en-tête `Idempotency-Key` : rejeu de la même clé → `{"indexed_chunks": 0}` (no-op anti-duplication). Seul `mask_pii` reste non câblé (décision produit).
- **Observabilité** : `app.observability` fournit `METRICS` (compteurs en mémoire) et `record_span("op", **attrs)` (context manager qui chronométre un bloc et émet un log JSON `span`). Sans dépendance externe (abstrait Langfuse/Prometheus) ; en prod, rebrancher le corps en gardant la signature. Nommer les compteurs/spans hiérarchiquement (`generator.primary.success`). `FallbackGenerator` est instrumenté ainsi. Les tests réinitialisent l'état via une fixture autouse `METRICS.reset()` (`tests/conftest.py`).
