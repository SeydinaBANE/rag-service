...
- **Gouvernance** : `app.governance` fournit `RBACPolicy`/`Permission`/`Principal` (RBAC), `mask_pii`, `IdempotencyGuard` et `AuditLog`. Seul `IdempotencyGuard` est branché pour l'instant : `/rag/index` accepte un en-tête `Idempotency-Key` ; un rejeu de la même clé renvoie `{"indexed_chunks": 0}` (no-op, évite la duplication de chunks). Le guard vit dans `Container.index_idempotency`. RBAC et `mask_pii` sont prêts mais non câblés — RBAC attend une couche d'auth fournissant un `Principal` (cf. `prod-api-hardening`), le masquage est une décision produit.
